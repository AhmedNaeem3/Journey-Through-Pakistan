...
function StatusViewerModal({ groups, groupIndex, onClose, onChangeGroup, currentUser, onStatusUpdate, initialStatusId }) {
  ...
  // Debug: Log if group or items are missing
  if (!group) {
    console.warn('StatusViewerModal: No group found for index', groupIndex);
  }
  if (group && (!items || items.length === 0)) {
    console.warn('StatusViewerModal: Group has no items', group);
  }
  if (item && !item.mediaUrl) {
    console.warn('StatusViewerModal: Item has no mediaUrl', item);
  }
              ...
              >
                {item && item.mediaUrl ? (
                  <img 
                    ...
                    alt="status"
                    onError={(e) => {
                      console.error('Failed to load status image:', item.mediaUrl);
                      e.target.style.display = 'none';
                ...
                {(!item || !item.mediaUrl) && (
                  <div className="text-center text-white py-5" style={{ 
                    display: (!item || !item.mediaUrl) ? 'flex' : 'none',
                    alignItems: 'center',
                    justifyContent: 'center',
                    ...
                    {item ? 'No image available' : 'No status'}
                  </div>
                )}
...
function PostCard({ post, onToggleLike, onAddComment, onHashtagClick, onPostUpdate, onPostDelete, currentUser, onPostSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newSavedState = data?.saved || false;
      setIsSaved(newSavedState);
      // Update user's savedPosts in context
      if (onPostSave) {
        onPostSave(post._id, newSavedState);
      }
      // Show notification if saved (tags will be processed in background)
      if (newSavedState && post?.hashtags && post.hashtags.length > 0) {
        // Notification will be shown by parent component if available
        console.log('Post saved! Tags will be processed for recommendations:', post.hashtags);
      }
    } catch (err) {
...
function NotificationsCenter({ notifications, onClose, navigate, setNotifications, user, setUser, onOpenStatus }) {
  const handleNotificationClick = (n) => {
    if (n.type === 'status_message' && n.status) {
      ...
      onClose();
    } else if (n.actor && n.type !== 'friend_request' && n.type !== 'admin_announcement') {
      // Don't allow opening admin profiles from user dashboard
      const isAdmin = n.actor?.isAdmin || n.type === 'admin_announcement';
      if (!isAdmin) {
                ...
                <div className="d-flex flex-column gap-2">
                  ...
                  const handleAddFriend = async () => {
                    try {
                      ...
                      const response = await apiSendFriendRequest(uid);
