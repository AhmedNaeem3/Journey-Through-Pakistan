...
export default function LandmarkResult() {
  ...
  useEffect(() => {
    const fetchAutoDescription = async () => {
      if (result?.name && result?.landmark_found && !autoDescription) {
        ...
        try {
          // Build precise location context using exact data from image detection
          let locationContext = '';
          if (result.location?.lat && result.location?.lng) {
            locationContext = `\n\nEXACT LOCATION IDENTIFIED FROM IMAGE:\n- Name: ${result.name}\n- Coordinates: ${result.location.lat}, ${result.location.lng}`;
            if (result.address) {
              locationContext += `\n- Address: ${result.address}`;
            }
            locationContext += `\n- Country: Pakistan`;
          } else if (result.address) {
            locationContext = `\n\nEXACT LOCATION IDENTIFIED FROM IMAGE:\n- Name: ${result.name}\n- Address: ${result.address}, Pakistan.`;
          }
          
          // Build enhanced prompt that emphasizes using the EXACT location data from image detection
          let prompt = `A user uploaded an image, and our image analysis system identified the following EXACT location from that image:\n\n`;
          prompt += `IDENTIFIED PLACE NAME: "${result.name}"\n`;
          if (result.location?.lat && result.location?.lng) {
            prompt += `IDENTIFIED COORDINATES: ${result.location.lat}, ${result.location.lng}\n`;
          }
          if (result.address) {
            prompt += `IDENTIFIED ADDRESS: ${result.address}\n`;
          }
          prompt += `\nCRITICAL INSTRUCTIONS:\n`;
          prompt += `1. Describe THIS SPECIFIC location identified from the image - "${result.name}" at coordinates ${result.location?.lat || 'N/A'}, ${result.location?.lng || 'N/A'}\n`;
          prompt += `2. DO NOT provide a generic description. Use the exact name, coordinates, and address provided above.\n`;
          prompt += `3. The coordinates ${result.location?.lat || 'N/A'}, ${result.location?.lng || 'N/A'} represent the EXACT location detected in the image.\n`;
          prompt += `4. Describe what "${result.name}" is at this specific location, not a general description.\n`;
          prompt += `\nInclude information about:\n`;
          prompt += `- What "${result.name}" is at coordinates ${result.location?.lat || 'N/A'}, ${result.location?.lng || 'N/A'}\n`;
          ...
          prompt += `- Why people visit this specific place\n`;
          prompt += `- Interesting facts about this exact location\n`;
          prompt += `\nFormat the response with clear sections and headings. `;
          prompt += `Remember: Describe the EXACT location identified from the image, not a generic description.`;
      ...
      <div className="container-fluid py-3">
        <div className="card shadow-sm">
          <div className="card-body text-center">
            ...
            <p className="text-muted mb-4">We couldn't identify the place in the image. Try searching manually:</p>
            <div className="d-flex gap-2 justify-content-center">
              <button className="btn btn-primary" onClick={() => navigate("/landmark")}>
                Try Again
              </button>
              <button className="btn btn-outline-primary" onClick={() => navigate("/search?q=&tab=places")}>
  ...
  const { landmark_found, name, location, confidence, distance, address, types, rating, nearest_places, labels, landmarkId, place_id, photos: resultPhotos, description, detection_method, detection_confidence, method, visual_similarity, web_entities } = result;
    ...
    <div className="container-fluid py-3 landmark-result-page">
      ...
      <div className="row g-3">
            ...
            <div className="card shadow-sm mb-3">
              <div className="card-body">
                    ...
                    <div className="d-flex flex-column align-items-end gap-1">
                      ...
                      {visual_similarity && (
                        <span className="badge bg-secondary" style={{ fontSize: '0.7rem' }}>
                          Identified using visual similarity
                        </span>
                      )}
                    </div>
                  )}
                ...
                {visual_similarity && (detection_method === 'web_detection' || method === 'web_detection_text_search') && (
                  <div className="alert alert-info d-flex align-items-center gap-2 mb-3" style={{ fontSize: '0.875rem' }}>
                    <span>ℹ️</span>
                    <span>This place was identified using visual similarity to images found on the web.</span>
                  </div>
                )}
        ...
        <div className="col-12 col-lg-4">
          ...
          ) : (!landmark_found && nearest_places && nearest_places.length > 0) ? (
            <div className="card shadow-sm mb-3">
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <button 
                    ...
                    onClick={() => navigate("/search?q=&tab=places")}
                  >
                    Search Manually
                  </button>
                </div>
                <p className="text-muted small mb-3">We couldn't identify the exact place. Here are nearby places you might be looking for:</p>
      ...
      <GeminiModal
        ...
        temperature={0.7}
        landmarkName={name}
        location={location ? { lat: location.lat, lng: location.lng, address: address } : null}
        autoFetch={false}
      />
    </div>
  );
