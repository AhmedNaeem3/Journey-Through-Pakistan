...
import { getPost, addComment, toggleLike, sharePost, updatePost, deletePost, reportPost, reportComment, toggleSavePost } from "../api/postsApi.jsx";
...
export default function PostDetail() {
        ...
        if (mounted) {
          ...
          if (currentUser?.savedPosts) {
            const saved = currentUser.savedPosts.some(
              (id) => id === data._id || id === data._id?.toString()
            );
            setIsSaved(saved);
          }
        }
  ...
  const handleToggleSave = async () => {
    ...
    try {
      ...
      if (data?.saved) {
        showNotification("Post saved! Tags are being processed for personalized recommendations.", 'success');
      } else {
        showNotification("Post unsaved", 'success');
      }
    } catch (err) {
      showNotification(err?.response?.data?.message || "Failed to save post", 'error');
    } finally {
      setSaving(false);
    }
  };
            ...
            <div className="card-body">
              ...
              <div className="d-grid gap-2">
                ...
                <button 
                  ...
                  className={`btn btn-primary-subtle ${isSaved ? 'active' : ''}`}
                  disabled={saving}
                >
                  <FiBookmark className="me-1" fill={isSaved ? 'currentColor' : 'none'} />
                  {saving ? 'Saving...' : (isSaved ? 'Saved' : 'Save Post')}
                </button>
              </div>
            </div>
          </div>
