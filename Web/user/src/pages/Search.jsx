...
import { getPlaceAutocomplete, getPlaceByGoogleId } from '../api/placesApi.jsx';
import { UserCardSkeleton, PostCardSkeleton, GroupCardSkeleton } from '../components/SkeletonLoader.jsx';
import { FiUsers, FiLock, FiGlobe, FiTag, FiMapPin, FiSearch, FiLoader } from 'react-icons/fi';
...
export default function Search() {
  ...
  const [activeTab, setActiveTab] = useState(tabParam || "all"); // all | people | posts | places | groups
  
  // Update activeTab when tab param changes
  useEffect(() => {
    if (tabParam && ['all', 'people', 'posts', 'places', 'groups'].includes(tabParam)) {
  ...
  const placeSuggestionsRef = useRef(null);
  ...
  // Handle click outside for place suggestions
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        placeSuggestionsRef.current &&
        !placeSuggestionsRef.current.contains(event.target) &&
        placeSearchInputRef.current &&
        !placeSearchInputRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    ...
    if (activeTab !== 'places' || !query || query.trim().length === 0) {
      setPlaceSuggestions([]);
      setShowPlaceSuggestions(false);
      return;
    }
    ...
    debounceTimerRef.current = setTimeout(async () => {
      try {
        const response = await getPlaceAutocomplete(query.trim());
        if (response.data?.success && Array.isArray(response.data.suggestions)) {
          setPlaceSuggestions(response.data.suggestions);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Don't search users/posts/groups when searching places
      if (activeTab === 'places') {
        setLoading(false);
        return;
      }
  ...
  const handlePlaceSelect = async (suggestion) => {
    try {
      ...
      if (errorHint) {
        toast.error(`${errorMessage}. ${errorHint}`, { autoClose: 8000 });
      } else {
  ...
  function friendButton(u) {
    ...
    const handleAddFriend = async () => { 
      setFriendActionLoadingId(u._id); 
      try {
        console.log('Sending friend request to:', u._id);
        const response = await sendFriendRequest(u._id);
        console.log('Friend request response:', response);
        
        if (response.data) {
          if (user.sentRequests) {
            user.sentRequests.push(u._id);
          } else {
            user.sentRequests = [u._id];
          }
          setImmed();
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to send friend request. Please try again.';
        alert(errorMessage);
      } finally {
    ...
    <div className="container-fluid py-3">
      <div className="d-flex flex-column gap-2 mb-3">
        <div className="position-relative">
          <input 
            ...
            value={query} 
            onChange={(e)=>{ setQuery(e.target.value); navigate(`/search?q=${encodeURIComponent(e.target.value)}`) }} 
            className="form-control" 
            placeholder={activeTab === 'places' ? "Search for a place (e.g., Badshahi Mosque, Lahore)" : "Search people, posts, places, groups..."}
            onFocus={() => {
              if (activeTab === 'places' && placeSuggestions.length > 0) {
                setShowPlaceSuggestions(true);
              }
            }}
          />
          {activeTab === 'places' && loadingPlaces && (
          ...
          {activeTab === 'places' && showPlaceSuggestions && placeSuggestions.length > 0 && (
            <div ref={placeSuggestionsRef} className="position-absolute w-100 bg-white border rounded shadow-lg" style={{ top: '100%', zIndex: 1000, maxHeight: '300px', overflowY: 'auto', marginTop: '4px' }}>
              {placeSuggestions.map((suggestion, index) => (
                <button
                  key={suggestion.place_id || index}
                  type="button"
                  className="btn btn-light w-100 text-start d-flex align-items-center gap-2 p-2 border-0"
                  onClick={() => handlePlaceSelect(suggestion)}
                  style={{ borderRadius: 0 }}
                >
                  <FiMapPin className="text-muted" />
                  <div className="flex-grow-1">
                    <div className="small fw-semibold">{suggestion.main_text}</div>
                    {suggestion.secondary_text && (
                      <div className="xsmall text-muted">{suggestion.secondary_text}</div>
                    )}
                  </div>
                </button>
              ))}
          ...
          {activeTab === 'places' && showPlaceSuggestions && !loadingPlaces && placeSuggestions.length === 0 && query.trim().length > 0 && (
            <div className="position-absolute w-100 bg-white border rounded shadow-lg" style={{ top: '100%', zIndex: 1000, marginTop: '4px', padding: '12px' }}>
              <div className="text-muted small text-center">No places found</div>
            </div>
          )}
        </div>
        <div className="btn-group">
          ...
          <button onClick={()=>setActiveTab('places')} className={`btn btn-light border ${activeTab==='places'?'active':''}`}><FiMapPin className="me-1" />Places</button>
      ...
      {!query || query.trim().length===0 ? (
        ...
        <div className="text-muted small text-center py-4">
          {query.trim().length > 0 
            ? (loadingPlaces ? 'Searching places...' : 'Select a place from suggestions above')
