 ...
 * @param {object} location - Location object with lat, lng, address (optional)
...
const GeminiModal = ({
  ...
  const fetchGeminiResponse = async (questionPrompt = null) => {
    ...
    // Build the full prompt with landmark context and location if available
    let fullPrompt = finalPrompt;
    
    // Add location context for accurate responses
    let locationContext = '';
    if (location?.lat && location?.lng) {
      ...
      locationContext += `\n- This location is in Pakistan.`;
    } else if (location?.address) {
      locationContext = `\n\nLocation: ${location.address}, Pakistan.`;
    }
    
    if (landmarkName) {
      if (!finalPrompt.toLowerCase().includes(landmarkName.toLowerCase())) {
        fullPrompt = `About ${landmarkName}${locationContext ? locationContext : ' in Pakistan'}: ${finalPrompt}`;
      } else if (locationContext) {
        // Add location context even if landmark name is already in prompt
        fullPrompt = `${finalPrompt}${locationContext}`;
      }
    } else if (locationContext) {
      fullPrompt = `${finalPrompt}${locationContext}`;
    }
