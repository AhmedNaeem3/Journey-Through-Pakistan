...
// 1. GOOGLE_MAPS_API_KEY (as per user requirements)
// 2. GOOGLE_PLACES_API_KEY (existing codebase convention)
...
export const getGooglePlacesAPIKey = async () => {
  ...
  let key = 
    ...
    process.env.GOOGLE_MAPS_API_KEY ||  // User-requested env var name
    process.env.GOOGLE_PLACES_API_KEY || // Existing convention
    await getGoogleAPIKey(); // Fallback to general Google API key
  
  // Debug logging only when key is missing (reduce noise)
  if (!key || key.trim().length === 0) {
    console.warn("⚠️ Google Places API Key not found:");
    console.warn("- From settings.googlePlacesApiKey:", !!settings.googlePlacesApiKey);
    console.warn("- From process.env.GOOGLE_MAPS_API_KEY:", !!process.env.GOOGLE_MAPS_API_KEY);
    console.warn("- From process.env.GOOGLE_PLACES_API_KEY:", !!process.env.GOOGLE_PLACES_API_KEY);
  }
