...
export const createPost = async (req, res) => {
  try {
    ...
    // Update user interests when creating a post (non-blocking)
    if (hashtags && hashtags.length > 0) {
      try {
        const { updateUserInterests } = await import('../utils/interestTracker.js');
        await updateUserInterests(userId, hashtags, 1, "post_created");
      } catch (interestError) {
        console.error("Error updating interests on post creation:", interestError.message);
...
export const toggleSavePost = async (req, res) => {
  try {
    ...
    if (isSaved) {
      ...
      // Decrement tag weights in user interest profile
      const UserInterest = (await import("../models/userInterest.models.js")).default;
      const userInterest = await UserInterest.findOne({ user: userId });
      if (userInterest && post.hashtags && post.hashtags.length > 0) {
        for (const tag of post.hashtags) {
          const normalizedTag = tag.toLowerCase().trim();
          const currentWeight = userInterest.tagWeights.get(normalizedTag) || 0;
          if (currentWeight > 0) {
            userInterest.tagWeights.set(normalizedTag, currentWeight - 1);
            // Sync with plain object
            if (userInterest.tagWeightsObj) {
              userInterest.tagWeightsObj[normalizedTag] = currentWeight - 1;
              if (userInterest.tagWeightsObj[normalizedTag] <= 0) {
                delete userInterest.tagWeightsObj[normalizedTag];
              }
            }
            userInterest.totalSavedPosts = Math.max(0, userInterest.totalSavedPosts - 1);
          }
        }
      ...
      // Extract tags and update user interest profile
      if (post.hashtags && post.hashtags.length > 0) {
        const Place = (await import("../models/place.models.js")).default;
        const { processTagAndFetchPlaces } = await import("../services/googleMapsService.js");
        const { updateUserInterests } = await import("../utils/interestTracker.js");
        
        // Update user interests using utility function
        await updateUserInterests(userId, post.hashtags, 1, "post_saved");
        
        // Fetch places from Google Maps (async, don't wait)
        for (const tag of post.hashtags) {
          processTagAndFetchPlaces(tag)
            .then(async (places) => {
              if (!places || places.length === 0) {
                console.log(`No places found for tag: ${tag}`);
                return;
              }
              
              console.log(`Found ${places.length} places for tag: ${tag}`);
              
              // Save places to database (avoid duplicates using googlePlaceId)
              let savedCount = 0;
              for (const placeData of places) {
                try {
                  if (!placeData.googlePlaceId) {
                    console.warn(`Skipping place ${placeData.name} - no googlePlaceId`);
                    continue;
                  }
                  ...
                  savedCount++;
                  console.log(`✅ Saved place: ${placeData.name} (${placeData.media?.length || 0} photos) from tag: ${tag}`);
                } catch (placeError) {
                  console.error(`❌ Error saving place ${placeData.name}:`, placeError.message);
                  if (placeError.stack) {
                    console.error('Stack:', placeError.stack);
                  }
                }
              }
              
              console.log(`✅ Successfully saved ${savedCount}/${places.length} places for tag: ${tag}`);
            })
            .catch((error) => {
              console.error(`❌ Error processing tag ${tag} for places:`, error.message);
              if (error.stack) {
...
export const toggleLike = async (req, res) => {
  try {
    ...
    if (hasLiked) {
      ...
      // Update user interests when liking a post
      if (post.author.toString() !== userId) {
        try {
          const { updateUserInterests, extractTagsFromPost } = await import('../utils/interestTracker.js');
          const tags = extractTagsFromPost(post);
          if (tags.length > 0) {
            await updateUserInterests(userId, tags, 0.5, "post_liked"); // 0.5 weight for likes
          }
        } catch (interestError) {
          console.error("Error updating interests on like:", interestError.message);
          // Non-critical, continue
        }
