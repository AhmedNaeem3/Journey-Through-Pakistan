...
export const verifyOtp = async (req, res) => {
    try {
        ...
        if (!user.otpVerify || !user.otpExpiry) return res.status(400).json({ message: 'No OTP to verify' });
        if (Date.now() > new Date(user.otpExpiry).getTime()) return res.status(400).json({ message: 'OTP expired' });
