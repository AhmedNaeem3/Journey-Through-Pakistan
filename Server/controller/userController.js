...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    // Convert to strings for comparison (ObjectId vs string)
    ...
    // Check if already friends (convert ObjectIds to strings for comparison)
    const isFriend = user.friends.some(f => {
      const friendId = f?._id ? f._id.toString() : f.toString();
      return friendId === targetUserIdStr;
    });
    if (isFriend) {
    ...
    const requestSent = user.sentRequests.some(r => {
      const requestId = r?._id ? r._id.toString() : r.toString();
      return requestId === targetUserIdStr;
    });
    if (requestSent) {
    ...
    const requestReceived = user.friendRequests.some(r => {
      const requestId = r?._id ? r._id.toString() : r.toString();
      return requestId === targetUserIdStr;
    });
    if (requestReceived) {
      return res.status(400).json({ message: 'That user already sent you a request.' });
    }
    ...
    // Normalize interests field before saving (fix for corrupted data)
    const normalizeInterests = (interests) => {
      ...
      for (const item of interests) {
        if (Array.isArray(item)) {
          ...
          normalized.push(...item.filter(i => typeof i === 'string'));
        } else if (typeof item === 'string') {
          normalized.push(item);
        }
      }
      // Remove duplicates and validate against enum
      const USER_INTERESTS = ["history", "nature", "culture", "food", "adventure"];
      return [...new Set(normalized.filter(i => USER_INTERESTS.includes(i.toLowerCase())))];
    };
    
    // Normalize interests for both users before saving
    if (user.interests) {
      user.interests = normalizeInterests(user.interests);
    }
    if (target.interests) {
      target.interests = normalizeInterests(target.interests);
    }
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    // Check if request exists (convert ObjectIds to strings for comparison)
    const hasRequest = user.friendRequests.some(r => (r._id || r).toString() === requestUserIdStr);
    if (!hasRequest) return res.status(400).json({ message: 'No such friend request.' });

    user.friendRequests = user.friendRequests.filter(u => (u._id || u).toString() !== requestUserIdStr);
    requestUser.sentRequests = requestUser.sentRequests.filter(u => (u._id || u).toString() !== userIdStr);
...
export const cancelFriendRequest = async (req, res) => {
  try {
    ...
    // Check if request exists (convert ObjectIds to strings for comparison)
    const hasRequest = user.sentRequests.some(r => (r._id || r).toString() === targetUserIdStr);
    if (!hasRequest) return res.status(400).json({ message: 'No such sent request.' });

    user.sentRequests = user.sentRequests.filter(u => (u._id || u).toString() !== targetUserIdStr);
    target.friendRequests = target.friendRequests.filter(u => (u._id || u).toString() !== userIdStr);
    await user.save();
    await target.save();
...
export const unfriend = async (req, res) => {
  try {
    ...
    // Check if friends (convert ObjectIds to strings for comparison)
    const isFriend = user.friends.some(f => (f._id || f).toString() === targetUserIdStr);
    if (!isFriend) return res.status(400).json({ message: 'Not friends.' });
    
    user.friends = user.friends.filter(u => (u._id || u).toString() !== targetUserIdStr);
    friend.friends = friend.friends.filter(u => (u._id || u).toString() !== userIdStr);
    await user.save();
    await friend.save();
