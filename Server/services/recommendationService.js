...
export async function getPersonalizedRecommendations(userId, userLocation) {
  ...
  // Get interests from both user.interests (predefined) and userInterest profile (from saved posts)
  const userInterest = await UserInterest.findOne({ user: userId }).lean();
  
  // Get tag weights as plain object (for MongoDB aggregation)
  let tagWeightsObj = {};
  if (userInterest) {
    // Use tagWeightsObj if available, otherwise convert Map
    if (userInterest.tagWeightsObj) {
      tagWeightsObj = userInterest.tagWeightsObj;
    } else if (userInterest.tagWeights) {
      // Convert Map to plain object
      if (userInterest.tagWeights instanceof Map) {
        for (const [key, value] of userInterest.tagWeights.entries()) {
          tagWeightsObj[key.toLowerCase()] = value;
        }
      } else {
        // If already an object (from lean()), use it directly
        tagWeightsObj = userInterest.tagWeights;
      }
    }
  }
  
  const savedPostTags = Object.keys(tagWeightsObj);
  
  // Combine predefined interests with tags from saved posts
  const interests = [
  ...
  pipeline.push({
    $addFields: {
      ...
      // Also consider tag weights from saved posts
      _interestMatchCount: {
        $size: {
          $setIntersection: [
            uniqueInterests,
      ...
      // Calculate weighted interest score based on tag frequency
      _interestWeight: {
        $reduce: {
          input: { $ifNull: ["$tags", []] },
          initialValue: 0,
            ...
            $add: [
              "$$value",
              {
                $cond: [
                  ...
                  // Get weight from tagWeightsObj, default to 1
                  {
                    $ifNull: [
                      {
                        $getField: {
  ...
  pipeline.push({
    $addFields: {
      // Interest score: base score + weighted boost
      interestScore: {
        $add: [
          {
            $cond: [{ $gt: ["$_interestMatchCount", 0] }, WEIGHTS.interest, 0],
          },
          // Boost based on tag weights (normalized, max +20 points)
          {
            $min: [
              { $multiply: ["$_interestWeight", 2] },
              20,
            ],
          },
        ],
